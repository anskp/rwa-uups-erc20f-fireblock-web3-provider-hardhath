...
var encodeReceivedBundledReports = (reports) => {
...
var updateReserves = (evmConfig, runtime2, totalReserveScaled, lastUpdated) => {
  ...
  runtime2.log(`Updating reserves totalReserveScaled ${totalReserveScaled.toString()}`);
...
var doPOR = (runtime2) => {
  ...
  const totalReserveScaled = BigInt(Math.floor(reserveInfo.totalReserve * 1000000000000000000));
  runtime2.log(`TotalReserveScaled ${totalReserveScaled.toString()}`);
  for (const evmConfig of runtime2.config.evms) {
    runtime2.log(`Updating reserves on chain ${evmConfig.chainName}`);
    updateReserves(evmConfig, runtime2, totalReserveScaled, reserveInfo.lastUpdated);
  }
