...
const encodeReceivedDecimaldReports = (reports: ReceivedDecimaldReport[]): Hex => {
...
const updateNAV = (evmConfig: EVMConfig, runtime: Runtime<Config>, navScaled: bigint): string => {
    ...
    runtime.log(`Updating NAV navScaled ${navScaled.toString()}`)
...
const doNAV = (runtime: Runtime<Config>): string => {
    ...
    const navScaled = BigInt(Math.floor(navInfo.nav * 1e18))
