...
var configSchema = exports_external.object({
  ...
  dataIdHex: exports_external.string(),
  evms: exports_external.array(exports_external.object({
    dataFeedsCacheAddress: exports_external.string(),
...
var fetchNAVInfo = (sendRequester, config) => {
  ...
  const navResp = JSON.parse(responseText);
  const nav = navResp._aggregatedCollateral - navResp._totalOwedM;
  return {
    nav
  };
};
var encodeReceivedDecimaldReports = (reports) => {
...
var hexToBytes32RightPadded = (input) => {
  ...
  if (byteLen > 32) {
    throw new Error(`hex string decodes to ${byteLen} bytes, which exceeds 32 bytes`);
  }
  const padded = hex.padEnd(64, "0");
  return "0x" + padded;
};
var updateNAV = (evmConfig, runtime2, navScaled) => {
  ...
  runtime2.log(`Updating NAV navScaled ${navScaled.toString()}`);
  const reportData = encodeReceivedDecimaldReports([
    {
      dataId: runtime2.config.dataIdHex,
...
var doNAV = (runtime2) => {
  runtime2.log(`fetching nav url ${runtime2.config.url}`);
  const httpCapability = new cre.capabilities.HTTPClient;
  const navInfo = httpCapability.sendRequest(runtime2, fetchNAVInfo, ConsensusAggregationByFields({
    nav: median
  }))(runtime2.config).result();
  runtime2.log(`NAVInfo ${safeJsonStringify(navInfo)}`);
  const navScaled = BigInt(Math.floor(navInfo.nav * 1000000000000000000));
  runtime2.log(`NavScaled ${navScaled.toString()}`);
  for (const evmConfig of runtime2.config.evms) {
    runtime2.log(`Updating NAV on chain ${evmConfig.chainName}`);
    updateNAV(evmConfig, runtime2, navScaled);
  }
